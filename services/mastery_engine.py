...
def update_student_mastery(student_id: str, skill_id: str, is_correct: bool, error_type: str = None) -> str:
    """Upgrades or downgrades a student's mastery state based on an answer."""
    res = supabase.table("student_mastery").select("id, current_streak, error_patterns").eq("student_id", student_id).eq("skill_id", skill_id).execute()
