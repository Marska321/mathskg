...
url: str = "https://dbeymbofoledmubfyzmz.supabase.co"
key: str = "sb_publishable_Ft9zN_KPc2LDVbGKcWj_HA_4kOYErKY"
...
def seed_database():
    ...
    for skill in foundation_skills:
        try:
            ...
            print(f"  Error on {skill['skill_id']}: {e}")

    print("\nWiring Dependency Edges...")
    for edge in foundation_edges:
        try:
            # We use upsert to avoid duplicate edge crashes
            ...
            print(f"  Error linking {edge['skill_id']}: {e}")
