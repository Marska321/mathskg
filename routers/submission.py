...
def get_next_skill(student_id: str):
    ...
    estimate_rows_response = (
        supabase.table('diagnostic_skill_estimates')
        .select('skill_id, student_mastery_status, source')
        .eq('student_id', student_id)
        .execute()
    )
    estimate_rows = estimate_rows_response.data or []
    ...
    decision = decide_next_skill_policy(
        mastery_rows,
        edges,
        diagnostic_estimate_rows=estimate_rows,
    )
